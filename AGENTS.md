...
Across all projects and languages, keep non-secret configuration in a config
file rather than environment variables. Only sensitive secrets, credentials,
tokens, API keys, and similar private values belong in environment variables or
secret stores. If a value is not secret, put it in the project's appropriate
configuration file, regardless of the language or config file format.
